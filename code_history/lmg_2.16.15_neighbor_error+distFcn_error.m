...
monolayers_per_iteration=1*(10/setup.mesh_size);
...
while input_iteration_number <= total_iterations
    ...
    for i=1:setup.source_number
        if setup.dimension==3
            ...
            generation_rate(i,:,:,:)=power_in*eval(['setup.intensity' num2str(i)])*pabs(i,:,:,:)/(hc/eval(['pabs' num2str(i) '.lambda']))*1e-4; %j/(s*cm)*(m^-2)*j*(1e-2 m/cm)^2=cm^-3
        end
    end
    
    %quick fix for same wavelength sources. Source weighting is included in setup.lsf
    gen_flag=0;
