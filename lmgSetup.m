...
if strfind(ls,'setup_parameters.mat')>0
    ...
    if isempty(javachk('desktop'))
        ...
        sampleinfo = inputdlg({ ...
            ...
            'Source 2 Wavelength (nm) (0 for profile import):'...   %16
            'Source 2 Polarization (deg):'...                       %17
            'Source 2 Phase (deg):'...                              %18
            'Distribution Function Exponent(1=linear, 2=quadratic):'}, ...%19
        ...
        if str2num(sampleinfo{10})==2;
            ...
            parameters.phase2=str2num(sampleinfo{18});
            if size(parameters.phase2,2)==1, parameters.phase2=parameters.phase2*ones(1,sim_number); end
            
            parameters.dist_fcn_exp=str2num(sampleinfo{19});
            if size(parameters.dist_fcn_exp,2)==1, parameters.dist_fcn_exp=parameters.dist_fcn_exp*ones(1,sim_number); end
        ...
        % Load intensity profile of source2 if wavelength is set to zero
        if parameters.source_number(1)==2 && parameters.wavelength2(1)==0
            ...
            while flag==0
                ...
                [FileName,PathName] = uigetfile({'*.txt;*.csv'},...
                    'Select source intensity file (wavelength units in nm, odd number of points):',...
                    'MultiSelect','off');
                source2_profile=importdata([PathName FileName]);
                if mod(size(source2_profile,1),2),flag=1; end
            end
            
            
            parameters.wavelength2=median(source2_profile(:,1));
            if size(parameters.wavelength2,2)==1, parameters.wavelength2=parameters.wavelength2(:,ones(1,sim_number)); end
            
            parameters.source_bandwidth2=max(source2_profile(:,1))-min(source2_profile(:,1));
            if size(parameters.source_bandwidth2,2)==1, parameters.source_bandwidth2=parameters.source_bandwidth2*ones(1,sim_number); end
            
            parameters.source_points2=size(source2_profile,1);
            if size(parameters.source_points2,2)==1, parameters.source_points2=parameters.source_points2*ones(1,sim_number); end
            
        else
            parameters.source_bandwidth2=zeros(1,sim_number);
        end
        ...
        temp=input(['Enter phase of source_2 for simulation(s) (deg): '],'s');
        parameters.phase2=str2num(temp);
        if size(parameters.phase2,2)==1, parameters.phase2=parameters.phase2*ones(1,sim_number); end
...
% save source2_profile if using
if isfield(parameters,'source_bandwidth2')
    if parameters.source_bandwidth2~= 0,save('source2_profile','source2_profile'),end
end
...
for i=1:sim_number
    ...
    copyfile([starting_dir '/lmg_update.lsf'],pwd)
    copyfile([starting_dir '/lmg_extract.lsf'],pwd)
